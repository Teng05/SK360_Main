    ...
    <div class="w-64 bg-red-600 text-white flex flex-col p-3 overflow-y-auto">
        ...
        <nav class="space-y-1 text-xs">
            @foreach($menuItems as $item)
                ...
                <a href="{{ $item['link'] }}"
                   class="flex items-center gap-2 p-2 rounded-lg {{ $isActive ? 'bg-red-500' : 'hover:bg-red-500' }}">

                    <span class="{{ $isActive ? 'bg-yellow-400 text-red-600' : 'bg-red-400' }} p-1 rounded">
                        {!! $item['icon'] !!}
                    </span>
