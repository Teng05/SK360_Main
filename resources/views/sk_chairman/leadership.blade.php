    ...
    <div class="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
        ...
        <form method="POST" action="{{ route('sk_chairman.leadership.store') }}" class="p-6 space-y-4">
            ...
            <div class="flex gap-3 pt-4">
                <button type="button" onclick="toggleModal('addCouncilorModal')" class="flex-1 py-3 text-xs font-black uppercase text-gray-400">
                    Cancel
                </button>

                <button type="submit" class="flex-1 bg-red-600 py-3 rounded-xl text-xs font-black uppercase text-white shadow-lg">
                    Save Councilor
                </button>
            </div>
        </form>
    ...
    <div class="bg-white rounded-3xl w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <form method="POST" action="{{ route('sk_chairman.leadership.bulk-councilors') }}" class="p-6">
                ...
                @foreach ($oldCouncilors as $index=>$councilor)
                    <div class="bulk-councilor-row rounded-2xl border bg-gray-50 p-4">
                        <div class="flex justify-between items-center mb-3">
                            ...
                            <button type="button" class="removeCouncilorRow {{ $loop->first ? 'hidden' : '' }} text-xs font-bold text-red-500">
                                Remove
                            </button>
                        </div>
            ...
            <div class="mt-5 flex justify-between gap-3">
                <button id="addCouncilorRow" type="button" class="rounded-xl border px-4 py-3 text-xs font-black text-gray-500 hover:bg-gray-50">
                    + Add Another Councilor
                </button>

                <button type="submit" class="rounded-xl bg-red-600 px-6 py-3 text-xs font-black uppercase text-white">
                    Save All Councilors
                </button>
            </div>
        </form>
...
<div id="addSecretaryModal" class="hidden fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
        <div class="bg-red-600 p-6 text-white">
            ...
            <p class="text-[10px] opacity-80 uppercase font-bold">Create the secretary account for Barangay {{ $barangayName }}.</p>
        </div>
        ...
        <form method="POST" action="{{ route('sk_chairman.leadership.secretary.store') }}" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="text" name="secretary_first_name" value="{{ old('secretary_first_name') }}" required class="w-full border-b-2 border-gray-100 focus:border-red-500 outline-none py-2 text-sm font-bold">
                </div>

                <div>
                    <label class="text-[10px] font-black text-gray-400 uppercase">Last Name</label>
                    <input type="text" name="secretary_last_name" value="{{ old('secretary_last_name') }}" required class="w-full border-b-2 border-gray-100 focus:border-red-500 outline-none py-2 text-sm font-bold">
                </div>
            </div>

            <div>
                <label class="text-[10px] font-black text-gray-400 uppercase">Email</label>
                <input type="email" name="secretary_email" value="{{ old('secretary_email') }}" required class="w-full border-b-2 border-gray-100 focus:border-red-500 outline-none py-2 text-sm font-bold">
            </div>

            <div>
                <label class="text-[10px] font-black text-gray-400 uppercase">Phone</label>
                <input type="text" name="secretary_phone" value="{{ old('secretary_phone') }}" class="w-full border-b-2 border-gray-100 focus:border-red-500 outline-none py-2 text-sm font-bold">
            </div>
            ...
            <div class="rounded-xl bg-yellow-50 px-4 py-3 text-xs text-yellow-700">
                The secretary will receive an email to set their password and activate the account.
            </div>

            <div class="flex gap-3 pt-3">
                <button type="button" onclick="toggleModal('addSecretaryModal')" class="flex-1 py-3 text-xs font-black uppercase text-gray-400">
                    Cancel
                </button>

                <button type="submit" class="flex-1 bg-red-600 py-3 rounded-xl text-xs font-black uppercase text-white">
                    Create Account
                </button>
            </div>
        </form>
...
<div id="editSecretaryModal" class="hidden fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
        ...
        <form method="POST" action="{{ route('sk_chairman.leadership.secretary.update',$secretary['user_id']) }}" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="flex gap-3 pt-3">
                <button type="button" onclick="toggleModal('editSecretaryModal')" class="flex-1 py-3 text-xs font-black uppercase text-gray-400">
                    Cancel
                </button>
...
<div id="addTreasurerModal" class="hidden fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
        <div class="bg-red-600 p-6 text-white">
            ...
            <p class="text-[10px] opacity-80 uppercase font-bold">Treasurer information only. No SK360 account will be created.</p>
        </div>
        ...
        <form method="POST" action="{{ route('sk_chairman.leadership.treasurer.store') }}" class="p-6 space-y-4">
            ...
            <div>
                <label class="text-[10px] font-black text-gray-400 uppercase">Full Name</label>
                <input type="text" name="treasurer_name" value="{{ old('treasurer_name') }}" required class="w-full border-b-2 border-gray-100 focus:border-red-500 outline-none py-2 text-sm font-bold">
            </div>

            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="text-[10px] font-black text-gray-400 uppercase">Email</label>
                    <input type="email" name="treasurer_email" value="{{ old('treasurer_email') }}" class="w-full border-b-2 border-gray-100 focus:border-red-500 outline-none py-2 text-sm font-bold">
                </div>

                <div>
                    <label class="text-[10px] font-black text-gray-400 uppercase">Phone</label>
                    <input type="text" name="treasurer_phone" value="{{ old('treasurer_phone') }}" class="w-full border-b-2 border-gray-100 focus:border-red-500 outline-none py-2 text-sm font-bold">
                </div>
            </div>

            <div>
                <label class="text-[10px] font-black text-gray-400 uppercase">Term</label>
                <input type="text" name="treasurer_term" value="{{ old('treasurer_term','2023-2026') }}" class="w-full border-b-2 border-gray-100 focus:border-red-500 outline-none py-2 text-sm font-bold">
            </div>

            <div class="flex gap-3 pt-3">
                <button type="button" onclick="toggleModal('addTreasurerModal')" class="flex-1 py-3 text-xs font-black uppercase text-gray-400">
                    Cancel
                </button>
        ...
        <main class="p-8 overflow-y-auto h-full bg-gray-50">
            ...
            <div class="flex justify-between items-end mb-8">
                ...
                <div class="flex items-center gap-2">
                    <button type="button" onclick="toggleModal('bulkCouncilorModal')" class="bg-gray-800 hover:bg-gray-900 text-white px-5 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg transition-all">
                        &#128101; Bulk Add
                    </button>

                    <button type="button" onclick="toggleModal('addCouncilorModal')" class="bg-red-600 hover:bg-red-700 text-white px-5 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg transition-all">
            ...
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 mb-8">
                <div class="flex items-center justify-between gap-3 mb-8 border-b border-gray-50 pb-4">
                    ...
                    <div class="flex items-center gap-2">
                        @if ($canAddSecretary)
                            <button type="button" onclick="toggleModal('addSecretaryModal')" class="rounded-xl bg-blue-600 hover:bg-blue-700 px-4 py-2 text-[9px] font-black uppercase text-white">
                                + Add Secretary
                            </button>
                        @endif

                        @if (!$treasurer)
                            <button type="button" onclick="toggleModal('addTreasurerModal')" class="rounded-xl bg-yellow-500 hover:bg-yellow-600 px-4 py-2 text-[9px] font-black uppercase text-white">
                                + Add Treasurer
                            </button>
                        @endif
                    </div>
                ...
                <div class="grid grid-cols-1 gap-4">
                    @foreach ($executives as $member)
                        ...
                        <div class="flex items-center gap-6 p-5 rounded-2xl border {{ $officerBorder }} transition group relative">                            
                            ...
                            @if ($isSecretary)
                                <div class="relative">
                                    <button type="button" class="secretary-menu-btn rounded-lg px-3 py-2 text-xl text-gray-400 hover:bg-gray-100">
                                        &#8942;
                                    </button>

                                    <div class="secretary-menu hidden absolute right-0 top-10 z-40 w-44 rounded-xl border bg-white shadow-xl py-1">
                                        <button type="button" onclick="toggleModal('editSecretaryModal')" class="block w-full text-left px-4 py-2 text-xs hover:bg-gray-50">
                                            Edit Details
                                        </button>

                                        @if ($pending)
                                            <form method="POST" action="{{ route('sk_chairman.leadership.secretary.resend',$member['user_id']) }}" onsubmit="return confirm('Send a new setup link to {{ $member['email'] }}?');">
                                                @csrf

                                                <button type="submit" class="block w-full text-left px-4 py-2 text-xs font-semibold text-blue-600 hover:bg-blue-50">
                                                    Resend Setup Link
                                                </button>
                                            </form>
                                        @else
                                            <form method="POST" action="{{ route('sk_chairman.leadership.secretary.toggle-status',$member['user_id']) }}">
                                                @csrf
                                                @method('PATCH')

                                                <button type="submit" class="block w-full text-left px-4 py-2 text-xs hover:bg-gray-50">
                                        ...
                                        <form method="POST" action="{{ route('sk_chairman.leadership.secretary.destroy',$member['user_id']) }}" onsubmit="return confirm('Delete this secretary account?');">
                                            @csrf
                                            @method('DELETE')

                                            <button type="submit" class="block w-full text-left px-4 py-2 text-xs font-semibold text-red-600 hover:bg-red-50">
                                                Delete Account
                                            </button>
                                        </form>
                                    </div>
                            ...
                            @if ($isTreasurer && !empty($member['council_id']))
                                <form method="POST" action="{{ route('sk_chairman.leadership.destroy',$member['council_id']) }}">
                                    ...
                                    <button type="submit" onclick="return confirm('Remove this treasurer?')" class="text-gray-300 hover:text-red-600 transition">
                                        &#128465;
                                    </button>
                                </form>
                            @endif
                        ...
                        <div class="bg-gray-50/50 p-5 rounded-2xl border border-gray-100 flex items-center gap-4 hover:shadow-md hover:bg-white transition group relative">
                            ...
                            @if (!empty($member['council_id']))
                                <form method="POST" action="{{ route('sk_chairman.leadership.destroy',$member['council_id']) }}" class="absolute top-4 right-4">
                                    ...
                                    <button type="submit" onclick="return confirm('Remove this councilor?')" class="text-[10px] text-gray-300 hover:text-red-600 opacity-0 group-hover:opacity-100 transition">
                                        &#128465;
                                    </button>
                                </form>
                            @endif
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    // Secretary Menu
    document.querySelectorAll('.secretary-menu-btn').forEach((btn)=>{
        btn.addEventListener('click',(e)=>{
            e.stopPropagation();

            const menu=btn.nextElementSibling;

            document.querySelectorAll('.secretary-menu').forEach((other)=>{
                if(other!==menu){
    ...
    document.addEventListener('click',(e)=>{
        ...
        document.querySelectorAll('.secretary-menu').forEach((menu)=>{
    ...
    // Open correct modal after validation
    @if ($errors->councilorAdd->any())
        document.getElementById('addCouncilorModal')?.classList.remove('hidden');
    @elseif ($errors->bulkCouncilors->any())
        document.getElementById('bulkCouncilorModal')?.classList.remove('hidden');
    @elseif ($errors->secretaryAdd->any())
        document.getElementById('addSecretaryModal')?.classList.remove('hidden');
    ...
    @elseif ($errors->treasurerAdd->any())
        document.getElementById('addTreasurerModal')?.classList.remove('hidden');
    @endif
