...
<script>
    ...
    // Existing signed mobile calls do not use web-session attendance endpoints.
    const joinAttendanceUrl=@json(request()->routeIs('mobile.meetings.call') ? null : ($joinAttendanceRoute ?? route('meetings.call-attendance.join',$meeting->meeting_id)));
    const leaveAttendanceUrl=@json(request()->routeIs('mobile.meetings.call') ? null : ($leaveAttendanceRoute ?? route('meetings.call-attendance.leave',$meeting->meeting_id)));
    ...
    const recordCallLeave=async()=>{
        if(!joinedCall || !leaveAttendanceUrl) return;
