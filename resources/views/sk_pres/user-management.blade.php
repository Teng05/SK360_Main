    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto bg-gray-50 p-8">
            ...
            @if(($activeTab ?? 'current') === 'current')
                ...
                <!-- Filters Bar -->
                <form method="GET"
                    action="{{ route('sk_pres.user-management') }}"
                    ...
                    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div class="relative w-full lg:max-w-xl">
                            ...
                            <input type="text"
                                ...
                                placeholder="Search by name, email, barangay, phone, role or term..."
                                autocomplete="off">
                        </div>
                ...
                <div class="space-y-4">
                    ...
                    @foreach($userGroups as $index=>$group)
                        ...
                        <details
                            class="current-user-group rounded-2xl border border-gray-100 bg-white shadow-sm"
                            ...
                            <div class="space-y-3 border-t border-gray-100 px-5 py-4 bg-gray-50/30">
                                ...
                                @forelse($group['users'] as $groupUser)
                                    ...
                                    @php
                                        ...
                                        $initials=strtoupper(
                                            substr($groupUser->first_name ?? 'U',0,1).
                                            substr($groupUser->last_name ?? '',0,1)
                                        );
                                        ...
                                        $isPending=
                                            (int)($groupUser->is_verified ?? 0) === 0;

                                        $isReappointment=
                                            $isPending &&
                                            ($groupUser->role ?? '') === 'sk_chairman' &&
                                            (int)($groupUser->completed_terms_count ?? 0) > 0;
                                                ...
                                                <div class="action-menu hidden absolute right-0 top-8 z-40 w-48 rounded-xl border border-gray-100 bg-white py-1.5 shadow-xl">
                                                    ...
                                                    @if($isPending)
                                                        ...
                                                        <form
                                                            ...
                                                            method="POST"
                                                            onsubmit="return confirm('{{ $isPresident ? 'Cancel this pending President succession? The current President will continue for this administration.' : ($isReappointment ? 'Cancel this Chairman reappointment? Historical service records will remain preserved.' : 'Delete this pending account permanently?') }}');">
                                                            ...
                                                            <button type="submit"
                                                                ...
                                                                {{ $isPresident ? 'Cancel Succession' : ($isReappointment ? 'Cancel Reappointment' : 'Delete Pending Account') }}
                                                            </button>
                                                        </form>
                                                    ...
                                                    @elseif(!$isPresident)
                                                        ...
                                                        <form
                                                            action="{{ route('sk_pres.user-management.toggle-status',$userId) }}"
                                                            method="POST">
                                                        ...
                                                        <form
                                                            action="{{ route('sk_pres.user-management.archive',$userId) }}"
                                                            method="POST"
            ...
            @else
                ...
                <div class="mb-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-xs text-gray-500 mt-1">
                            Complete leadership history by administration. Chairman reappointment creates a new pending term and requires a new password setup.
                        </p>
                    </div>
                    ...
                    <form id="historyFilterForm"
                        ...
                        <div>
                            <label class="mb-1 block text-[10px] font-black uppercase text-gray-400">
                                Term
                            </label>
                        ...
                        <div>
                            ...
                            <select name="history_role"
                                ...
                                <option value="sk_president"
                                    {{ $selectedHistoryRole === 'sk_president' ? 'selected' : '' }}>
                                    SK President
                                </option>

                                <option value="sk_chairman"
                                    {{ $selectedHistoryRole === 'sk_chairman' ? 'selected' : '' }}>
                                    SK Chairman
                                </option>

                                <option value="sk_secretary"
                                    {{ $selectedHistoryRole === 'sk_secretary' ? 'selected' : '' }}>
                                    SK Secretary
                                </option>

                                <option value="sk_treasurer"
                                    {{ $selectedHistoryRole === 'sk_treasurer' ? 'selected' : '' }}>
                                    SK Treasurer
                                </option>

                                <option value="sk_councilor"
                                    {{ $selectedHistoryRole === 'sk_councilor' ? 'selected' : '' }}>
                                    SK Councilor
                                </option>
                            </select>
                        </div>
                    ...
                    <div class="relative mt-4">
                        ...
                        <input type="text"
                            class="live-user-search w-full rounded-xl bg-gray-50 py-3 pl-10 pr-4 text-sm text-gray-700 outline-none ring-1 ring-transparent focus:ring-red-200 transition"
                            placeholder="Search name, email, phone, barangay, position or term..."
                            autocomplete="off">
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    @forelse($historyGroups as $index=>$historyGroup)
                        ...
                        <details
                            class="history-group rounded-2xl border border-gray-200 bg-white shadow-sm"
                            data-default-open="{{ $index === 0 ? '1' : '0' }}"
                            {{ $index === 0 ? 'open' : '' }}>
                            ...
                            <summary class="flex cursor-pointer list-none items-center justify-between px-5 py-4 hover:bg-gray-50 transition">
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-[10px] text-gray-400">
                                        {{ $historyGroup['members']->count() }} official(s) recorded for this administration
                                    </p>
                                </div>

                                <div class="flex items-center gap-3">

                                    @if($historyGroup['key'] !== 'federation')

                                        <span class="rounded-full bg-gray-100 px-3 py-1 text-[9px] font-black text-gray-500">

                                            {{ $historyGroup['executives']->count() }}
                                            Executive{{ $historyGroup['executives']->count() === 1 ? '' : 's' }}
                                        </span>

                                        <span class="rounded-full bg-purple-50 px-3 py-1 text-[9px] font-black text-purple-600">

                                            {{ $historyGroup['councilors']->count() }}
                                            Councilor{{ $historyGroup['councilors']->count() === 1 ? '' : 's' }}
                                        </span>
                            ...
                            <div class="border-t border-gray-100 bg-gray-50/30 p-5">
                                ...
                                @php
                                    $historySections=$historyGroup['key'] === 'federation'
                                        ? [[
                                            'key'=>'federation',
                                            'label'=>'Federation Administration',
                                            'description'=>'Federation-level leadership record',
                                            'items'=>$historyGroup['executives'],
                                        ]]
                                        : [[
                                            'key'=>'executives',
                                            'label'=>'Executive Officers',
                                            'description'=>'Chairman, Secretary and Treasurer',
                                            'items'=>$historyGroup['executives'],
                                        ],[
                                            'key'=>'councilors',
                                            'label'=>'SK Councilors',
                                            'description'=>'Council members recorded for this administration',
                                            'items'=>$historyGroup['councilors'],
                                        ]];
                                @endphp
                                ...
                                <div class="space-y-5">
                                        ...
                                        @if($section['items']->isNotEmpty())
                                            ...
                                            <section class="history-section"
                                                ...
                                                <div class="mb-3 flex items-center justify-between border-b border-gray-100 pb-2">
                                                    ...
                                                    <span class="rounded-full bg-white px-2.5 py-1 text-[9px] font-black text-gray-500 ring-1 ring-gray-100">
                                                        {{ $section['items']->count() }}
                                                    </span>
                                                </div>

                                                <div class="{{ $section['key'] === 'councilors' ? 'grid grid-cols-1 gap-3 lg:grid-cols-2' : 'space-y-3' }}">

                                                    @foreach($section['items'] as $historyUser)

                                                        @php
                                                            $historyName=trim(
                                                                ($historyUser->first_name ?? '').
                                                                ' '.
                                                                ($historyUser->last_name ?? '')
                                                            );

                                                            $nameParts=preg_split('/\s+/',trim($historyName)) ?: [];
                                                            ...
                                                            $roleStyle=match($historyUser->role ?? ''){
                                                                'sk_president'=>[
                                                                    'avatar'=>'bg-red-600',
                                                            ...
                                                            <div class="flex items-start gap-3">
                                                                ...
                                                                <div class="min-w-0 flex-1">
                                                                    ...
                                                                    <div class="flex flex-wrap items-center gap-2">
                                                                        ...
                                                                        <span class="rounded-full px-2.5 py-1 text-[8px] font-black uppercase {{ $roleStyle['badge'] }}">
                                                                            {{ $historyRole }}
                                                                        </span>
                                                                    </div>
                                                                    ...
                                                                    @if(($historyUser->role ?? '') === 'sk_chairman' && $currentAdministration)

                                                                        <div class="mt-3">

                                                                            @if($historyUser->can_reappoint ?? false)

                                                                                <form
                                                                                    action="{{ route('sk_pres.user-management.reappoint',$historyUser->official_term_id) }}"
                                                                                    method="POST"
                                                                                    onsubmit="return confirm(@js('Reappoint '.$historyName.' as SK Chairman for '.$currentAdministration->start_year.' - '.$currentAdministration->end_year.'? Their old password will be invalidated and a new password setup link will be sent by email.'));">

                                                                                    @csrf

                                                                                    <button type="submit"
                                                                                        class="rounded-lg bg-green-600 px-3 py-2 text-[9px] font-black uppercase text-white hover:bg-green-700 transition">

                                                                                        Reappoint Chairman to
                                                                            ...
                                                                            @elseif(!empty($historyUser->reappointment_status))

                                                                                <span class="inline-flex rounded-lg bg-gray-100 px-3 py-2 text-[9px] font-black uppercase text-gray-500">

                                                                                    {{ $historyUser->reappointment_status }}
                                                                                </span>
                                    ...
                                    <p class="history-search-empty hidden text-xs text-gray-400 italic">
                    ...
                    @empty
                        ...
                        <div class="rounded-2xl border border-gray-200 bg-white p-8 text-center">
                            ...
                            <p class="mt-1 text-xs text-gray-400">
                                No completed leadership records were found for this administration or filter.
                            </p>
                        </div>
    ...
    <div class="w-full max-w-xl rounded-3xl bg-white p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div class="mb-6 flex items-center justify-between">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">
                    Create the next SK administration and choose the President handover.
                </p>
            </div>

            <button id="closeNewTermModal"
                type="button"
        ...
        <form
            ...
            <div>
                ...
                <p class="mb-2 text-xs font-black uppercase text-gray-500">
                    President for the New Administration
                </p>
                ...
                <label class="mb-2 flex cursor-pointer items-start gap-3 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3">
                    ...
                    <span>
                        <span class="block text-xs font-black text-gray-800">
                            Continue Current President
                        </span>
            ...
            <div id="newPresidentFields"
                ...
                <p class="text-xs font-black text-blue-700">
                    New President Account
                </p>

                <div class="grid grid-cols-2 gap-3">

                    <div>
                        <label class="mb-1 block text-[10px] font-black uppercase text-gray-500">
                            First Name
                        </label>

                        <input type="text"
                            name="new_president_first_name"
    ...
    <div class="w-full max-w-xl rounded-3xl bg-white p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form id="editUserForm"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <select id="edit_role"
                        ...
                        <option id="editPresidentRoleOption"
                            value="sk_president"
                            disabled>
                ...
                <div>
                    ...
                    <p id="pendingStatusNote"
                        class="hidden mt-1 text-[10px] text-yellow-600">
            ...
            <div>
                ...
                <select id="edit_barangay_id"
                    name="barangay_id"
                    class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-300">

                    <option value="">
                        Unassigned / Federation
                    </option>
...
<div id="importModal"
    ...
    <div class="w-full max-w-xl rounded-3xl bg-white p-6 shadow-2xl">
        ...
        <form action="{{ route('sk_pres.user-management.import') }}"
            method="POST"
    ...
    <div class="w-full max-w-6xl rounded-3xl bg-white p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div class="mb-6 flex items-center justify-between">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">
                    Create multiple SK Chairman accounts at once.
                </p>
            </div>
        ...
        <form action="{{ route('sk_pres.user-management.store-bulk-officials') }}"
            method="POST">
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    const updateUrlTemplate=@json(
        route('sk_pres.user-management.update',[
            'userId'=>'__ID__'
        ])
    );
    ...
    // Modal Controls
    if(openNewTermModal){
        openNewTermModal.addEventListener('click',()=>{
            showModal(newTermModal);
        });
    }

    if(closeNewTermModal){
        closeNewTermModal.addEventListener('click',()=>{
    ...
    const syncPresidentFields=()=>{
        const selected=
            document.querySelector('input[name="president_mode"]:checked')?.value ||
            'continue';

        const assigningNew=selected==='assign_new';

        if(newPresidentFields){
            newPresidentFields.classList.toggle(
                'hidden',
                !assigningNew
            );

            newPresidentFields.querySelectorAll('.new-president-input')
                .forEach((field)=>{
    ...
    // Official History Filters
    document.querySelectorAll('.history-auto-filter')
        .forEach((select)=>{
    ...
    if(addBulkRow && bulkRows){
        ...
        addBulkRow.addEventListener('click',()=>{
            ...
            const firstRow=
                bulkRows.querySelector('.bulk-row');
            ...
            const removeButton=
                newRow.querySelector('.removeBulkRow');
    ...
    function updateBulkTitles(){
        ...
        document.querySelectorAll('#bulkRows .bulk-row')
            .forEach((row,index)=>{

                const title=
                    row.querySelector('.bulk-title');
                ...
                const removeButton=
                    row.querySelector('.removeBulkRow');
    ...
    // Action Menu
    document.querySelectorAll('.action-menu-btn')
        .forEach((btn)=>{
    ...
    function filterUsers(value){
        ...
        if(activeTab==='current'){
            ...
            const userGroups=
                document.querySelectorAll('.current-user-group');

            const noSearchResults=
                document.getElementById('currentNoSearchResults');
            ...
            userGroups.forEach((group)=>{
                ...
                const countBadge=
                    group.querySelector('.group-count');

                const emptyMessage=
                    group.querySelector('.live-search-empty');

                const noUsersMessage=
                    group.querySelector('.no-users-message');
                ...
                if(noUsersMessage){
                    noUsersMessage.classList.toggle(
                        'hidden',
                        query!==''
                    );
                }
            ...
            const historyGroups=
                document.querySelectorAll('.history-group');

            const noSearchResults=
                document.getElementById('historyNoSearchResults');
            ...
            historyGroups.forEach((group)=>{
                ...
                const cards=
                    group.querySelectorAll('.history-card');

                const emptyMessage=
                    group.querySelector('.history-search-empty');
                ...
                group.querySelectorAll('.history-section')
                    .forEach((section)=>{

                        const sectionCards=
                            section.querySelectorAll('.history-card');
    ...
    @elseif($errors->editUser->any())
        ...
        @if($failedUserId)
            ...
            const failedRole=
                @json(old('role','sk_chairman'));

            const failedRoleSelect=
                document.getElementById('edit_role');

            const failedBarangaySelect=
                document.getElementById('edit_barangay_id');

            const failedIsPresident=
                failedRole==='sk_president';

            failedRoleSelect.querySelectorAll('option')
                .forEach((option)=>{
            ...
            failedRoleSelect.value=
                failedRole;

            failedBarangaySelect.disabled=
                failedIsPresident;

            failedBarangaySelect.value=
                @json(old('barangay_id',''));

            const failedStatusSelect=
                document.getElementById('edit_status');

            failedStatusSelect.value=
                @json(old('status','inactive'));
